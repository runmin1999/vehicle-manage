...
//        return new JSONObject().fluentPut("data", list);
    ...
    public JSONObject asearch0(String request) {
        List<Record> aSearch0 = recordDao.aSearch0();
        return new JSONObject().fluentPut("errorCode", 0).fluentPut("error", null)
                .fluentPut("data", aSearch0);
    }

    public void updateRecord2(ServletRequest servletRequest, String request) {
        Record record = updateRecord(servletRequest, request);
        recordDao.aUpdate2(record);
        recordDao.updateVehicle(record.getVehicleId());

    }

    public void updateRecord1(ServletRequest servletRequest, String request) {
        Record record = updateRecord(servletRequest, request);
        recordDao.aUpdate1(record);
//        recordDao.updateVehicle(record.getVehicleId());
    }

    public Record updateRecord(ServletRequest servletRequest, String request) {
        val p = Objects.requireNonNull(JSON.parseObject(request));
        JSONObject json = Objects.requireNonNull(p.getJSONObject("record"));
        Record record = JSONObject.toJavaObject(json, Record.class);
