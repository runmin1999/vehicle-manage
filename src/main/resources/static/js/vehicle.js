...
window.onload = function () {
    ...
    $(document).on("click", "#vehicleDisAll", function () {//获取所有数据
        ...
        $.ajax({
            ...
            url: 'http://localhost:10010/car/record/asearch',
            data: JSON.stringify({
                status1: 2
            }),
            // xhrFields: {
                ...
                if (data.errorCode == 0) {
                    ...
                    if (data.data) {
                        ...
                        for (let i in data) {
                            ...
                            str += "<td>" +
                                '<button class="manage-allow" >允许</button>' +
                                '<button class="manage-refuge" >拒绝</button>' +
                                "</td>";
                            str += "</tr>";
                        }
                        $("#vehicleTable").append(str);
    ...
    $(document).on("click", ".manage-allow", function () {
        ...
        $.ajax({
            ...
            dataType: 'JSON',
            contentType: "application/json",
            url: 'http://localhost:10010/car/record/updateRecord2',
            data: JSON.stringify({
                record: {
                    id: id,
                    vehicleId: vehicleId
    ...
    $(document).on("click", ".manage-refuge", function () {
        var id = $(this).parents("tr").children("td#data").text();
        var vehicleId = $(this).parents("tr").children("td#vehicleId").text();
        //console.log(id);
        $.ajax({
            ...
            dataType: 'JSON',
            contentType: "application/json",
            url: 'http://localhost:10010/car/record/updateRecord1',
            data: JSON.stringify({
                record: {
                    id: id,
                    vehicleId: vehicleId
